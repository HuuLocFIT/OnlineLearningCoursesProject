...
const URL_TEAM_API = "http://localhost:5000/api/teachers/";
...
const TeacherMember = ({ data }) => {
  return (
    <div className="member-item rounded-lg shadow-md transition-all">
...
const Teacher = () => {
  ...
  useEffect(() => {
    getListMember().then((listTeachers) => {
  ...
  return (
    <StyledTeamWrapper>
      <h1 className="member-title text-5xl text-center mt-[40px] mb-[40px]">
        Danh Sách Giáo Viên
      </h1>
      {loading && (
        <div className="list-member-wrapper grid grid-cols-3 gap-10 px-10 pb-10">
          <LoadingTeacherMember />
          <LoadingTeacherMember />
          <LoadingTeacherMember />
        </div>
      )}
      <div className="list-member-wrapper grid grid-cols-3 gap-10 px-10 pb-10">
        {!loading &&
          ...
          listTeachers.map((member) => {
            return <TeacherMember key={member._id} data={member} />;
          })}
      </div>
    </StyledTeamWrapper>
  );
