          ...
          lessons.map((lesson, index) => {
            return (
              <li key={index}>
                <div className="course-lesson-item px-[20px] py-[8px]">
                  ...
                  <div className="flex gap-x-3 z-10">
                    <div
                      ...
                      onClick={() => handelEditLesson(lesson.id)}
                    >
                      <FontAwesomeIcon
                        icon={faPencil}
                        className="text-[18px] text-[#4e4e4e]"
                      />
                    </div>
                    <div
                      className="w-[30px] h-[30px] rounded-full border-2 flex items-center justify-center"
                ...
                {lessonSelected === lesson.id && showDeleteLessonModal && (
                  <Modal
                  ...
                  >
                    <DeleteLessonConfirm
                      ...
                      id_chapter={lesson.id_chapter}
                      id_course={id_course}
                      closeModal={closeDeleteLessonModal}
                    />
                  </Modal>
                )}
              </li>
    ...
    <DetailStyled className="detail-course-container max-w-[1320px] mx-auto mb-[80px]">
                    ...
                    return (
                      <li key={index}>
                        <div className="px-[20px] py-[8px] border-b-2 border-b-[#ccc] bg-[#f7f8fa] cursor-pointer flex justify-between">
                          ...
                          <div>
                            <span
                              className=" ml-[10px] w-[30px] h-[30px] rounded-full border border-[#ccc] inline-flex items-center justify-center float-right"
                            ...
                            <span
                              className=" ml-[10px]  w-[30px] h-[30px] rounded-full border border-[#ccc] inline-flex items-center justify-center float-right"
                              onClick={() => handelEditChapter(chapter.id)}
                            >
                          ...
                          showEditChapterModal && (
                            ...
                            >
                              <EditChapterForm
                                id_chapter={chapter.id}
                                id_course={idCourse}
                                closeModal={closeEditChapterModal}
                              />
                            </Modal>
                          )}
                        {chapterSelected === chapter.id &&
                          showDeleteChapterModal && (
                            ...
                            >
                              <DeleteChapterConfirm
                                id_chapter={chapter.id}
                                id_course={idCourse}
                                closeModal={closeDeleteChapterModal}
                              />
                            </Modal>
                          )}
      ...
      {showAddChapterModal && (
        ...
        >
          <AddChapterForm
            id_course={idCourse}
            closeModal={closeAddChapterModal}
          />
        </Modal>
      )}
    </DetailStyled>
