...
const Checkout = () => {
  ...
  const checkoutRequest = async (list_courses) => {
    const response = await axios.post(
      `http://localhost:5000/api/courses/payment/${user._id}`,
      list_courses
    );
  ...
  const handleCheckout = () => {
    if (cart_products.length > 0) {
      const list_courses = cart_products.map((product) => product);
      setLoadingCheckout(true);
      checkoutRequest(list_courses).then((result) => {
  ...
  return (
    <CheckoutStyle className="my-[40px] max-w-[1320px] mx-auto">
      <h2 className="checkout-heading text-[46px] text-[var(--primary-color)] mb-[40px]">
        Check out
      </h2>
      <div>
        <div className="">
          ...
          <div>
            <div className="cart-container max-w-[1320px] mx-auto my-[40px] mb-[80px]">
                    ...
                    {cart_products.length > 0 &&
                      cart_products.map((product, index) => {
                        return (
                          <tr
                            key={index}
                            className="cart-table-row grid grid-cols-6 py-[20px]"
                          >
                            ...
                            <td className="my-auto ">
                              <button
                                className="px-[15px] py-[5px] text-[22px] bg-[#ccc] text-[var(--primary-color)]"
                                onClick={() => {
                              ...
                              <span className="text-[22px] px-[25px]">
                                {product.quantity}
                              </span>
                              <button
                                className="px-[15px] py-[5px] text-[22px] bg-[#ccc] text-[var(--primary-color)]"
              ...
              <div className="cart-bottom flex justify-between items-center mt-[40px]">
                <h3 className="font-semibold text-[20px] text-[var(--primary-color)]">
                  Total cost: {handleTotalBill()} VNĐ
                </h3>
              </div>
            </div>
            ...
            <div className="">
              <div>
                <div className="max-w-[500px]">
                  <h3 className="mb-[12px] text-[20px] font-semibold">
                    Payment Method
                  </h3>
                  ...
                  <div className="mt-[20px]">
                    ...
                    <input
                      ...
                      className="text-[#ccc] w-full py-[12px] px-[10px] text-[18px] mt-[6px] mb-[12px]"
                    />
                    ...
                    <button
                      type="submit"
                      className="ml-auto mt-[20px] px-[12px] py-[10px] bg-[var(--primary-color)] text-white"
